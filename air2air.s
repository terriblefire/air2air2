; ----------------------------< HARDWARE EQUATES >-----------------------------
		...
		jmp	go
		include	include/what_key.i ; include file of all the hardware equates
		include	include/hardware.i ; include file of all the key equates. 
...
plane_len	equ	44
line_len	equ	4*plane_len
stride_len	equ	3*plane_len ; 
no_lines	equ	232
...
sprite_lines	equ	16*13		; 12 Blocks down...
		...
		ds.b	62*line_len ; buffer to protect my music
...
sprite_info_structure	incbin	data/sprites_info.bin
song1		incbin	music/air2air.mod
song2		incbin	music/air2air2.mod
...
go		diskoff
	    ...
	    ; make everything AGA compatible
		move.l	gb_ActiView(a6),asm_oldview
		...
		move.l	#bus_error,08(a6)
		move.l	#address_err,12(a6)
		move.l	#illegal_inst,16(a6)
		move.l	#divide_zero,20(a6)

		move.l	$68(a6),asm_keyboard
		move.l	$6c(a6),asm_vblank
		move.l	#keyboard,$68(a6)
		move.l	#vblank,$6c(a6)

		clr.b	keycode

		move.w	#1,mt_endflag ; SJL - set the music system up to be intialised properly on startup
...
screen_mapped	dc.w	0

; -------------------------------< BLITWAIT >-------------------------------
		section code,CODE

	;; wait for the blitter to complete its operation
	;; Reformatted to work with PHXASM and VASM
	
blitwait	macro
		btst.b  #14-8,dmaconr
.waitloop\1	btst.b	#14-8,dmaconr
 		bne.s	.waitloop\1
		endm
...
copperlist	dc.w	bplcon0-custom,$4200	; It's safe here.
hrz_shift	dc.w	bplcon1-custom,$FFFF
		dc.w	bplcon2-custom,$0020

cop_dfst	dc.w	ddfstrt-custom,$0028	; No scroll thank you.
cop_dfsp	dc.w	ddfstop-custom,$00d0
cop_dwst	dc.w	diwstrt-custom,56*256+128
cop_dwsp	dc.w	diwstop-custom,255*256+193

	        ;setup the inter plane stride  
cop_mod1	dc.w	bpl1mod-custom, stride_len
cop_mod2	dc.w	bpl2mod-custom, stride_len
...
pal1		dc.b	255,01,255,00
		...
		dc.w	bpl1mod-custom, -396 ; -2*line_len - plane_len
		dc.w	bpl2mod-custom, -396 ; -2*line_len - plane_len

ripple		dc.b	03,01,255,00
		
		dc.w	bpl1mod-custom, -220 ; -1*line_len - plane_len
		dc.w	bpl2mod-custom, -220 ; -1*line_len - plane_len
...
exit		bsr	mt_end
		...
		move.l	asm_keyboard,$68(a6)
		move.l	asm_vblank,$6c(a6)
...
.delay		dbra	d1,.delay
		...
		move.l	d0,a0
		move.l  gb_copinit(a0),d0
		move.l  d0,cop1lch
		move.w  copjmp1,d0

		move.l	a0,a6
		move.l	asm_oldview,a1
...
.gap_loop	bsr	halt		
		...
		lea	bak_colours,a1
		move.l	bak_screen,a2
		lea	presents_pic,a0 ; CHANGE TO PRESENTS PIC >><><><><><><><><><<
...
.gap_loop	bsr	halt		
		...
		lea	bak_colours,a1
		move.l	bak_screen,a2
		lea	title_pic,a0 ; CHANGE TO PRESENTS PIC >><><><><><><><><><<
		bsr	show_pic
...
.gap_loop	bsr	halt		
		...
		lea	colours,a1
		move.l	bak_screen,a2
		lea	credits_pic,a0 ; CHANGE TO PRESENTS PIC >><><><><><><><><><<
		...
		lea	colours,a1
		move.l	bak_screen,a2
		lea	credits_pic2,a0 ; CHANGE TO PRESENTS PIC >><><><><><><><><><<
...
scroll_loop	bsr	halt
		...
		cmp.l	#448*65536,yscroll
		bmi	.okay
		
		move.l	#300*65536,ripst
		bsr	ripplecon
		move.l	#448*65536,yscroll
...
; ---------------------------------< MUSIC >----------------------------------
		...
		include include/jukebox.i
		include	music/replay.s ; music player
		
; ---------------------------------< MENU >----------------------------------
		
		include	include/menu.i
...
main_loop	bsr	halt
		...
		bne.s	.end_game
		cmp.b	#KB_ESC,keycode
		bne.s	.no_escape
		move.w	#1,game_fin
.no_escape	cmp.b	#KF_10,keycode
		bne.s	main_loop
...
; ---------------------------< AEROPLANES >----------------------------------
		...
		section code,CODE
plane1		cmp.w	#-16,spr_xpos(a0) ; So that if the plane goes out off screen (X) then it is returned on the other side
		bpl	.okay
	
		move.w	#334,spr_xpos(a0)
		
.okay		cmp.w	#336,spr_xpos(a0)
		bmi	.yako
		
		move.w	#-15,spr_xpos(a0)
	
.yako		tst.w	gravQuest
		bne.s	.fart
...
.no_croabh
		...
		move.w	#turn,spr_clock(a0)
		add.w	#1,spr_word1(a0)

.no_up		tst.b	key_q ; Rotate Anticlockwise
		beq.s	.no_down
...
.floor		cmp.w	#188,spr_ypos(a0)
		...
		rts
; ----------------------------------------------------------------------------
plane2		cmp.w	#-16,spr_xpos(a0) ; So that if the plane goes out off screen (X) then it is returned on the other side
		bpl	.okay
...
.no_up		btst.b	#joy_down,joyval2 ; Rotate Anticlockwise
		beq.s	.no_down
...
.no_down	btst.b	#joy_right,joyval2
		beq.s	.no_left
...
.no_left	btst.b	#joy_left,joyval2
		beq.s	.no_right
...
.floor		cmp.w	#188,spr_ypos(a0)
		...
		rts
; ---------------------------------------------------------------------------------------------------------------------------
plane3		cmp.w	#-16,spr_xpos(a0) ; So that if the plane goes out off screen (X) then it is returned on the other side
		bpl	.okay
...
.no_up		btst.b	#joy_down,joyval1 ; Rotate Anticlockwise
		beq.s	.no_down
...
.no_down	btst.b	#joy_right,joyval1
		beq.s	.no_left
...
.no_left	btst.b	#joy_left,joyval1
		beq.s	.no_right
...
.floor		cmp.w	#188,spr_ypos(a0)
		...
		move.w	#100*16,spr_type(a0)
		move.w	#108*16,spr_type(a1)

		move.w	#0,spr_word1(a0)
		clr.l	spr_long1(a0)
		clr.w	spr_stall(a0)
...
.no_hit		lea	plane_2,a1
		...
		lea	plane2,a3
		move.l	a2,spr_task(a0)
		move.l	a3,spr_task(a1)
		

.no_hit2	tst.l	spr_long1(a0)
		bne.s	.no_croabh
...
.no_ten
		...
		move.w	d1,spr_type(a1)
		lea	sprlist_len(a1),a1
		move.w	d0,spr_type(a1)
		
		rts
		
; --------------------------------------------------------------------------

scoreboard2	move.l	a0,a1
		clr.w	d0
		clr.w	d1
		move.w	score2,d0
.check		
...
deg45		equ	speed*59316  ;46341
deg45B		equ	speed*1092*45

deg90		equ	speed*65536+32768
deg90b		equ	speed*65536
...
title_pic	incbin	gfx/title.lbm
...
block_pic	incbin	gfx/background.iff ; Day Background
block_pic2	incbin	gfx/background2.iff ; Night BackGround
